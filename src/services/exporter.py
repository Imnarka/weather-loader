...
class Exporter:
    ...
    async def export_to_excel(self, session: AsyncSession):
        ...
        df = pd.DataFrame([(wd.temperature, wd.wind_direction, wd.wind_speed, wd.pressure, wd.precipitation) for wd in weather_data], 
                            columns=['Temperature', 'Wind Direction', 'Wind Speed', 'Pressure', 'Precipitation'])
